...
// `next dev` must run in development mode: launched from inside the
// coordinator's plantlab-web service the parent has NODE_ENV=production,
// which the dev server would otherwise inherit and 500 on every route. Any
// next-server that outlives Playwright's teardown is reaped by the
// collector's precise per-port pkill (see supportCollect.ts).
